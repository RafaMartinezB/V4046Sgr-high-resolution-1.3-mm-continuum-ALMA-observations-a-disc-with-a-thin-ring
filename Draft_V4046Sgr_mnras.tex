...
%\end{multline}
%where $F$ is the total flux of the blob, $d$ the distance of 71.48\,pc , $\lambda$ the wavelength, $\kappa_{1.3\,\mathrm{mm}}$ the opacity at 1.3\,mm and the temperature set at $T=500\,\mathrm{K}$. Then, its estimated mass would be around 0.12\,M$_{\earth}$.
...
Reproducing the radial and vertical structure of the V4046\,Sgr disc turned out to be challenging. We built the model in terms of the gas distribution, and with two main dust populations: large grains with radii from 0.3\,$\micron$ to 10\,mm that are vertically settled and dominate the total dust mass, and a population of smaller grains with radii ranging from 0.3 to 1.5\,$\micron$ that are uniformly mixed with the gas and reach higher regions above the mid-plane.% We also added a third special dust population of larger grains with a distribution tightly confined to the stellar vicinity, within 2\,au, and a special size range meant to account for the central emission present in the ALMA image. This last dust population has grains with radii ranging from 0.8 to 10\,mm, same upper limit as the large dust but is depleted of small grains.
...
%Rings and gaps are common structures present in protoplanetary discs, but only recent observations have allowed us to study them in detail. In the past years, observing systems with solar-type stars, that are not exceptionally bright or large, has not been the main focus of high resolution imaging. For that reason, using the DARTTS-S programme data, that collected DPI images for T Tauri Stars, together with the counterpart of ALMA continuum maps from DARTTS-A, we can search for substructures in that kind of systems as never before. Following that line, 
...
\begin{figure*}
	\includegraphics[width=\columnwidth]{SED_three_model_comparision.pdf}
        ...
        %the truncation radius due to dynamical binary-disc interaction.
