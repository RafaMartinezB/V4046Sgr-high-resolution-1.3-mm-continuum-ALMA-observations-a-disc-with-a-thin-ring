...
\documentclass[usenatbib,times]{mnras}
...
For the vertical structure, \citet{dOrazi} found flaring angles, i.e. $H/r$, of $\varphi$ = 6.2$\pm$0.6\,$\degr$ for the inner ring and $\varphi$ = 8.5$\pm$1.0\,$\degr$ for the outer one. Our model aims to reproduce those values by using two different flaring indices, $\psi_1$ and $\psi_2$. The separation between the two values were set at $r = 18$\,au with $\psi_1=0.2$ for Ring5 and Ring13, and $\psi_2=0.5$ for Ring24. The scale height is $H_\circ = 2.2$\,au at $r_\circ = 18$\,au. This is summarized in Fig.\ref{fig:profiles} bottom panel that shows the aspect ratio profile $h(r)=H(r)/r$. 
...
For Ring24 we used the same power-law as for the inner disc but scaled by an empirically obtained factor, $\delta_{\mathrm{sd}}(r)$= 4.2$\times 10^4$ for $r>18$\,au and $\delta_{\mathrm{sd}}(r)$= 0 for $r<18$\,au:
\begin{equation}
...
\end{equation}
where $\delta_{\mathrm{ld}}(r)$ ensures continuity
\begin{equation}
  \delta_{\mathrm{ld}}(r) =
  \begin{cases}
  0                 & r < 24.6\text{\,au} \\
...
S.C. acknowledges support from FONDECYT grant 1171624. S.P. acknowledges support from the Joint Committee of ESO and the Government of Chile and FONDECYT grant 1191934.
