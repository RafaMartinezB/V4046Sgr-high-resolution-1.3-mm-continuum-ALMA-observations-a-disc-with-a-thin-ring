...
\begin{figure*}
  \includegraphics[width=\textwidth]{hot_two_E_noice.pdf}
...
\begin{figure}
	\centering
	\includegraphics[width=\columnwidth]{SED_noice.png}
...
\end{equation}
where $H_\circ$ is the scale height at r = $r_\circ$, $\psi$ is the flaring index and $\chi$ is a scaling factor (in the range $0-1$) that mimics dust settling. For the gas and the small dust grains $\chi$ = 1 and for the millimetre-size dust population we assign a fixed $\chi$ = 1/5 for simplicity ***(now is not for simplicity).
...
\end{equation}
with $\delta_{\mathrm{sd}}$ = 5.3$\times 10^4$. But the inner wall of Ring24 follows a different behaviour. Between $R_\mathrm{in}$ and $R_\mathrm{peak}$, marking the beginning and the maximum density peak of the outer ring respectively, the gas density profile is multiplied by an additional factor
\begin{equation}
...
\begin{equation}
  ...
  2.65 \times10^{4} & 28.6 < r < 37\text{\,au} \\
  2.04 \times10^{5} & 37   < r < 64\text{\,au.} \\
  \end{cases}
\end{equation}
...
\begin{figure}
	\includegraphics[width=\columnwidth]{allprofiles_noice.png}
...
\begin{figure}
	\includegraphics[width=\columnwidth]{comp_fig_all_profiles_au_noice.pdf}
...
Using the large grains scale height of 0.31\,au at Ring13, and a measured width of 1.50\,au, the ring is $\sim$4.8 times more extended in radial than in vertical direction.
